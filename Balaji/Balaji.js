...
let mobileCover =
  [
    {
      "model": "Realme",
      "mobileName": "Pug In Winter Premium Glass Case for Realme X",
      "price": "₹499",
    ...
    {
      "model": "Realme",
      "mobileName": "Planet Play Printed Premium Glass Cover for Realme 7 Pro",
      "price": "₹499",
    ...
    {
      "model": "Realme",
      "mobileName": "House Of Dragon Designer Hard Cover Realme GT",
      "price": "₹199",
    ...
    {
      "model": "Realme",
      "mobileName": "Luffy Nika Premium Glass Case for Realme C25",
      "price": "₹499",
    ...
    {
      "model": "Realme",
      "mobileName": "Brave Lion Printed Premium Glass case for Realme Narzo 30 Pro",
      "price": "₹399",
    ...
    {
      "model": "Realme",
      "mobileName": "Power Of Lord Printed Premium Glass Realme 9i",
      "price": "₹399",
    ...
    {
      "model": "Realme",
      "mobileName": "Brave Lion Printed Premium Glass case forRealme 8s",
      "price": "₹399",
      "image": "https://images.bewakoof.com/t1080/cartoon-art-glass-case-for-realme-10-pro-5g-597850-1687271010-1.jpg"
    },
    {
      "model": "Realme",
      "mobileName": "Kitten Mandala Printed Premium Glass Case for Realme GT Neo 2",
      "price": "₹399",
    ...
    {
      "model": "Realme",
      "mobileName": "Luffy Nika Premium Glass Case for  Realme C21",
      "price": "₹399",
    ...
    {
      "model": "Realme",
      "mobileName": "Kitten Mandala Printed Premium Glass Case for Realme Narzo 50",
      "price": "₹299",
...
function createCard(data) {
  ...
  let carditm=document.createElement("div");
  ...
  carditm.className="Card-Item"
  span.innerText = data.model;
  span.style.display = "inline-block";
  btn.innerText = "Buy Now"
  btn.style.display = "inline-block";
  p.innerText = data.mobileName;
  h3.innerText = data.price;
  carditm.append(span,btn);
  card.append(image,carditm, p, h3);
  return card;
}
...
function appendData(items) {
 ...
 function showData(mobileCover) {
  let brandSelect = document.getElementById("Mobile-select").value;
  let DomMain = document.getElementById("DomMain");
  DomMain.innerHTML = ""; // Clear previous content

  mobileCover.forEach(item => {
      if (item.model.toLowerCase() === brandSelect.toLowerCase() || brandSelect === "") {
...
document.getElementById("Mobile-select").addEventListener("change", showData);


    showData(mobileCover);

function populateModels(selectedBrand) {
  let modelsDropdown = document.getElementById("Model-select");
  modelsDropdown.innerHTML = ""; // Clear previous options

  // Find models for the selected brand
  let models = mobileCover.filter(item => item.model === selectedBrand);
  
  // Add options to the dropdown
  models.forEach(model => {
      let option = document.createElement("option");
      option.text = model.mobileName;
      modelsDropdown.add(option);
  });
}

// Event listener for the change event of the Brand select dropdown
document.getElementById("Mobile-select").addEventListener("change", function() {
  let selectedBrand = this.value;
  if (selectedBrand !== "") {
      populateModels(selectedBrand);
      this.disabled = true; // Disable the Brand select dropdown
  }
});

function redirectToNewPage(selectedBrand) {
  // Filter the mobile cover data based on the selected brand
  let updatedData = mobileCover.filter(item => item.model === selectedBrand);

  // Encode the updated data as a JSON string
  let jsonData = encodeURIComponent(JSON.stringify(updatedData));

  // Redirect to the new page with the encoded JSON data as a query parameter
  window.location.href = "balaji1.html?data=" + jsonData;
}
...
document.getElementById("Mobile-select").addEventListener("change", function() {
  let selectedBrand = this.value;
  if (selectedBrand !== "") {
      redirectToNewPage(selectedBrand);
  }
});
